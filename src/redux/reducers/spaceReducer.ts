...
const actionsSpace = {
  setSpaceItem: (title: string, img: string | null) => ({ type: 'CRI/SPACE/SET_SPACE_ITEM', payload: { title, img } } as const)
}

export const getSpaceItem =
  (disable: (param: boolean) => void): ThunkType =>
  async (dispatch) => {
    disable(true)
    let timeoutId = setTimeout(
      () => dispatch(actionsSpace.setSpaceItem('Проблема на стороне сервера, пожалуйста, попробуйте позже. КиберНИИ.', '')),
      7000
    )
