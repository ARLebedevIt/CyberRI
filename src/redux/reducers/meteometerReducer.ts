...
import { UnitsTypes } from '../../components/Meteometer/MeteometerNav'
...
const actionsMeteometer = {
  setWeather: (
    ...
    ({
      type: 'CRI/METEOMETER/SET_WEATHER',
      payload: {
        temp,
        feelsLike,
        tempMin,
...
export const getWeather =
  (city: string, units: UnitsTypes, disable?: (param: boolean) => void): ThunkType =>
  async (dispatch) => {
    disable?.(true)
    ...
    dispatch(
      actionsMeteometer.setWeather(
        ...
        humidity,
        country,
        sunriseToUnix.toLocaleString(),
        sunsetToUnix.toLocaleString(),
        name,
        description,
        units,
        null
