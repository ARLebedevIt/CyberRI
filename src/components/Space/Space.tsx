        ...
        <div className="space__items">
          ...
          {img ? (
            ...
            <span className="space__text">
              Мироздание - разработка, позволяющая получить изображение из космоса на основании случайной генерации данных из базы
              NASA
            </span>
          )}
          <span className={`space__text ${title.length > 1 || '_hideTitle'}`}>{title}</span>
          <Button className="space__button" disabled={disable} onClick={() => setSpaceItem(setDisable)}>
            ∞
          </Button>
        </div>
      </div>
