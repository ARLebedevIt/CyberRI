...
const ProfileEditMode: FC<DescriptionProfileType & PropsType> = memo((props) => {
  ...
  const sendDescription = (
    values: DescriptionProfileType,
    onSubmitProps: {
      setStatus: (param: string | string[]) => void
      setSubmitting: (isSubmitting: boolean) => void
    }
  ) => {
    dispatch(saveDescripton(values, onSubmitProps.setStatus, onSubmitProps.setSubmitting, props.setEditMode, props.setLoading))
      ...
      {({ values, errors, handleChange, handleBlur, isValid, handleSubmit, isSubmitting }) => (
        <Form onSubmit={handleSubmit}>
          <div className={props.editMode ? 'description__edit__items' : 'description__items_bio'}>
            <label htmlFor="updatePhoto" className="description__label">
              Фотокарточка
            </label>
            <Input formik id="updatePhoto" type={'file'} className="description__input_photo" onChange={onMainPhotoSelected} />

            {errors.fullName && <span className="description__edit_errors">{errors.fullName}</span>}
            <Textarea
              formik
              label={'Имя'}
              autoFocus
            ...
            {errors.aboutMe && <span className="description__edit_errors">{errors.aboutMe}</span>}
            <Textarea
              formik
            ...
            <Button className="description__button" type={'submit'} disabled={!isValid || isSubmitting}>
              Сохранить
            </Button>
          </div>
        </Form>
