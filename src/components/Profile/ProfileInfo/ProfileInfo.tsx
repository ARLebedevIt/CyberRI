...
const ProfileInfo: FC<ProfileInfo> = ({ isOwner, loading, setLoading }) => {
    ...
    <div className="profile__content">
      <div className={`description__items ${editMode && '_descriptionEdit'}`}>
        ...
        <div className="description__items_bio">
          {editMode ? (
            <ProfileEditMode setLoading={setLoading} setEditMode={setEditMode} editMode={editMode} />
          ) : (
            <form>
              <div className="description__items_bio">
                ...
                <span>
                  {profile.lookingForAJobDescription ? `Навыки: ${profile.lookingForAJobDescription}` : `Навыки: Отсутствуют`}
                </span>
                <span>{profile.contacts.github ? `ГитХаб: ${profile.contacts.github}` : `ГитХаб: Отсутствует`}</span>
                <span>{profile.contacts.mainLink ? `Портфолио ${profile.contacts.mainLink}` : `Портфолио: Отсутствует`}</span>
                <span>{status ? `Статус: ${status}` : 'Статус: Отсутствует'}</span>
                {isOwner && (
                  <Button className="description__button" onClick={() => setEditMode(true)}>
                    <span>Править</span>
                  </Button>
                )}
              </div>
      ...
      {isOwner ? (
        <MyPosts profilePhoto={profile.photos.large} />
      ) : (
