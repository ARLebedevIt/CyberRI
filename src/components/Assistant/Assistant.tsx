...
const AssistantContainer = React.memo(() => {
  ...
  if (!isMicrophoneAvailable) {
    return (
      <span className="assistant__error">
        К сожалению, Вы не предоставили доступ к Микрофону. Хотим заверить, что у нас нет доступа к записям разговора между Вами и
        'Подручным', будьте спокойны. КиберНИИ.
      </span>
    )
  }
  return (
    <>
      <Page className={`assistant__content ${colorSchema}`}>
        ...
        <div className={`assistant__title ${openComands ? 'assistant__blur' : null}`}>
          <span>
            Подручный на данный момент <span id="assistant__status">{listening ? 'слушает Вас' : 'не слушает Вас'}</span>
          </span>
        </div>
        <div className={`assistant__items ${openComands ? 'assistant__blur' : null}`}>
          <div className={`assistant__item`}>
            {listening ? (
              <span className="assistant__transcript">{transcript}</span>
            ) : noteVisible ? (
              <span className="assistant__transcript">{note}</span>
            ) : (
              <div className="assistant__description">
                <span className="assistant__description_text">
                  Подручный - умный робот-помощник, который внимательно выслушает Вас и не оставит равнодушным. К сожалению, он
                  очень молчалив, но зато умеет выполнять <a onClick={() => setOpenComands(true)}>команды</a>.
                </span>
              </div>
            )}
          </div>
