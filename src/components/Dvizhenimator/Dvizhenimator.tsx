...
import { getImageSelector, getErrorImageSelector } from '../../redux/selectors/dvizhenimatorSelectors'
...
const Dvizhenimator: FC = memo(() => {
  ...
  const setPic = useCallback((input: string, offset: number, setDisabled: Dispatch<SetStateAction<boolean>>) => {
    ...
    <Page className="dvizhenimator__wrapper">
      ...
      <div className="dvizhenimator__items">
        <div className="dvizhenimator__text">
          ...
          <span>
            Движениматор 2.0 - многоуровневная система, использующая передовые вычислительные алгоритмы для визуализации вашей
            мысли! Перенесите поток сознания на поисковую строку...
          </span>
        </div>
        <div className="dvizhenimator__item">
          <div className="dvizhenimator__img">
            <img src={image ?? dvizhenimatorIMG} alt="Здесь могла быть ваша картинка" />
          </div>
        </div>
